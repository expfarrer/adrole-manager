    ...
    client.bind(bindDN, password, (err) => {
      ...
      client.search('DC=company,DC=com', opts, (err, res) => {
        ...
        res.on('searchEntry', (entry) => {
          console.log('Found entry:', entry.dn.toString());
          
          // Use entry.object to get parsed attributes
          if (entry.object && entry.object.memberOf) {
            const memberOfValues = Array.isArray(entry.object.memberOf) 
              ? entry.object.memberOf 
              : [entry.object.memberOf];
            
            groups = memberOfValues.map(dn => {
  ...
  return new Promise((resolve, reject) => {
    ...
    client.bind(bindDN, password, (err) => {
      ...
      client.search('DC=company,DC=com', opts, (err, res) => {
        ...
        res.on('searchEntry', (entry) => {
          console.log('Found group entry:', entry.dn.toString());
          
          // Use entry.object to get parsed attributes
          if (entry.object && entry.object.member) {
            const memberValues = Array.isArray(entry.object.member)
              ? entry.object.member
              : [entry.object.member];
            
            members = memberValues.map(dn => {
              const match = dn.match(/CN=([^,]+)/);
              return match ? `CN=${match[1]}` : dn;
            });
