...
setInterval(() => {
  ...
  console.log(`${colors.cyan}[${timestamp}] [HEARTBEAT] Server is running... (${connectionCount} connections handled)${colors.reset}`);
}, 20000);
...
server.on('connection', (socket) => {
  connectionCount++;
  const timestamp = new Date().toLocaleTimeString();
  console.log(`${colors.brightCyan}[${timestamp}] [CONNECTION] Client connected from: ${socket.remoteAddress} (Total: ${connectionCount})${colors.reset}`);
});
...
const mockUsers = {
 ...
 'tmconio': {
    dn: 'CN=Tobias Conio,OU=Users,DC=company,DC=com',
    attributes: {
      cn: 'Tobias Conio',
      sn: 'Conio',
      mail: 'tmconio@company.com',
      sAMAccountName: 'tmconio',
      memberOf: [
...
server.bind('DC=company,DC=com', (req, res, next) => {
  ...
  if (validCredentials[username] && validCredentials[username] === password) {
    ...
    console.log(`${colors.blue}[${timestamp}] [BIND] Expected password for ${username}: ${validCredentials[username] || '(user not found)'}${colors.reset}`);
    return next(new ldap.InvalidCredentialsError());
  }
});
...
server.search('DC=company,DC=com', (req, res, next) => {
  ...
  if (usernameMatch) {
    ...
    if (user) {
      console.log(`${colors.brightCyan}[${timestamp}] [SEARCH] ✓ Found user: ${username} with ${user.attributes.memberOf.length} groups${colors.reset}`);
      res.send({
        dn: user.dn,
  ...
  if (isGroupSearch && groupMatch) {
    ...
    if (group) {
      console.log(`${colors.brightCyan}[${timestamp}] [SEARCH] ✓ Found group: ${groupName} with ${group.attributes.member.length} members${colors.reset}`);
      res.send({
        dn: group.dn,
